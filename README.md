...
|----------|-------|------------|
| UTM Parameters | 5 | utm_source, utm_medium, utm_campaign, utm_term, utm_content |
| Tracking | 4 | Source_Identifier_URL, GA_Client_ID, WebsiteFormType, type |
| User Input | 4 | mobile, otp, client_first_name, selected_account |
| Referral | 2 | rcode, referralcode |
| Lead Source | 1 | leadSource |
| Partner Screening | 3 | Partner_Score, Partner_Status, Partner_Questions |
| System Generated | 3 | ipAddress, checksum, appSource |
| Backend API | 16 | clientFirstName, clientLastName, product, leadId, preSelectedPlan, email, leadCampaign, smsOTP, userId, urlParams, emailVerified, mobileVerified, skipOtpSource, emailConsent, mobileConsent, subSource |
| **Total** | **38** | |
...
```
User fills form (mobile + optional referral code)
         │
         ▼
┌────────────────────────────────┐
│  Frontend: kyc-oda-form.js     │
│  - Collects UTM from cookie    │
│  - Builds Source_Identifier_URL│
│  - Gets GA_Client_ID           │
│  - Determines WebsiteFormType  │
│  - Encodes all params          │
└────────────────────────────────┘
         │
         ▼
┌────────────────────────────────┐
│  API: CheckClientStatus        │
│  - Checks if user exists       │
│  - Status 0: New user (OTP)    │
│  - Status 1/2: Existing user   │
└────────────────────────────────┘
         │
         ▼
┌────────────────────────────────┐
...
│  API: verifyOtp                │
│  - Verifies OTP                │
│  - Passes all URLParams        │
│  - Includes partner data       │
└────────────────────────────────┘
         │
         ▼
┌────────────────────────────────┐
...
└────────────────────────────────┘
         ...
         ▼
┌────────────────────────────────┐
│  Redirect to KYC Journey       │
│  (or Thank You page for        │
│   rejected partners)           │
└────────────────────────────────┘
```
